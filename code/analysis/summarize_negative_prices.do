...
- clean up monthly summary of negative prices by node
- map to wind farms by lat long
...
mat rownames cmat = "\textbf{\underline{All nodes}}" "Mean" "Median" "95th pctile" ///
							...
							"\textbf{\underline{Near wind}}" "Mean" "Summer (mean)" "Post 2012 (mean)"  /// 
...
frmttable using "$outdir/tables/negative_price_summary.tex", statmat(cmat) varlabels replace ///
	ctitle("", "CAISO", "ERCOT", "ISONE", "MISO", "NYISO", "PJM") hlines(11{0}1) spacebef(1000010010001000) frag tex ///
...
cap confirm file "$repodir/generated_data/md_from_hhmy_aer_2016.dta"
if _rc!=0 { // only read in data the first time
	...
	sort region hour
	compress
	save "$repodir/generated_data/md_from_hhmy_aer_2016.dta", replace
}

*plot marginal damages from load along with PTC value, both in $/MWh
use "$repodir/generated_data/md_from_hhmy_aer_2016.dta", clear

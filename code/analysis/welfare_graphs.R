...
( p_bids <- mc_nonwind %>% rename(bid_nonwind = mc) %>% 
    ...
    ggplot(aes(d, value, color = var)) + geom_line(size = 1.75) +
    scale_color_manual(values=c("black","blue","red")) +
    annotate("text", x=505, y=150, label="MC(nonwind)", hjust = 0, size = 5, parse = TRUE) +
    annotate("text", x=505, y=50, label="S", hjust = 0, size = 5, parse = TRUE) +
    annotate("text", x=505, y=25, label="S(PTC)", hjust = 0, size = 5, parse = TRUE) +
    annotate("text", x=575, y=-15, label="Bids", hjust = 0, vjust = 1, size = 5) +
    geom_segment(aes(x = 100, y = -100, xend = 100, yend = 125), color = "black", 
                 linetype = "dotted", size = 1.75) +
    annotate("text", x=100, y=135, label="D[1]", size = 5, parse = TRUE) +
    geom_segment(aes(x = 300, y = -100, xend = 300, yend = 125), color = "black", 
                 linetype = "dotted", size = 1.75) +
    annotate("text", x=300, y=135, label="D[2]", size = 5, parse = TRUE) +
    geom_segment(aes(x = 400, y = -100, xend = 400, yend = 125), color = "black", 
                 linetype = "dotted", size = 1.75) +
    annotate("text", x=400, y=135, label="D[3]", size = 5, parse = TRUE) +
    theme_bw() +
...
policy_labs <- c("No PTC","PTC","PTC - Dispatch Only")
( p_mc2 <- facet_data %>%
    ...
    labs(x="Demand",y="Marginal Costs") + 
    scale_y_continuous(breaks = c(-subsidy,0),
                   labels = c(expression(phi),0)) +
